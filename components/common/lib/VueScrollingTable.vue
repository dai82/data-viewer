<template>
  <!-- vue-scrolling-tableをローカルに持ってきて機能拡張する -->
  <!-- https://github.com/richardtallent/vue-scrolling-table -->
	<table ref="table" class="scrolling" :class="{ scrolly: scrollVertical, scrollx: scrollHorizontal }" :style="tableStyle">
		...
		<tfoot v-if="includeFooter" ref="tfoot" name="tfoot" :class="{ scrollsync: syncFooterScroll }" :style="syncFooterScroll && scrollVertical ? stubScrollbarStyle : ''">
			<slot name="tfoot" />
		</tfoot>
	</table>
</template>
